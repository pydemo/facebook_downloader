...
class DownloaderFrame(wx.Frame):
    def __init__(self):
        ...
        #self.page_label = wx.StaticText(panel, label="Page Name:")
        self.page_name = wx.ComboBox(panel, choices=["Any", "ArtForUkraine", "SexyAI"], style=wx.CB_DROPDOWN)
    ...
    def ____log(self, message):
        wx.CallAfter(self.log_ctrl.AppendText, message + "\n")
    def log(self, message, color=wx.RED):
        def _log():
